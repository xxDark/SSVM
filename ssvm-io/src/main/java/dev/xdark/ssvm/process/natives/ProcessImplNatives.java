package dev.xdark.ssvm.process.natives;
...
import dev.xdark.ssvm.process.ProcessManager;
...
public class ProcessImplNatives {
	 ...
	 */
	public void init(VirtualMachine vm, FileManager fileManager, ProcessManager manager) {
		...
		if (createWindowsProcess != null) { // windows jdk 8
			...
			vmi.setInvoker(genericProcessImpl, "getStillActive", "()J", (ctx) -> {
				ctx.setResult(ProcessManager.STILL_ACTIVE); // needs to be -1 because it needs to be different from getExitCodeProcess
				return Result.ABORT;
			});
			...
			vmi.setInvoker(genericProcessImpl, "openForAtomicAppend", "(Ljava/lang/String;)J", (ctx) -> {
				try {
					fileManager.open(ops.readUtf8(ctx.getLocals().loadReference(0)), FileManager.APPEND);
				} catch (IOException e) {
			...
			if (unixProcessImpl == null) {
				...
				forkAndExec = genericProcessImpl.getMethod("forkAndExec",
					"(I[B[B[BI[BI[B[IZ)I");
				if (forkAndExec != null) {
					unixProcessImpl = genericProcessImpl;
				} else {
