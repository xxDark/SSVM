 ...
 * @author xDark
 */
public final class GetFieldByteProcessor implements InstructionProcessor<VMFieldInsnNode> {
	...
	public Result execute(VMFieldInsnNode insn, ExecutionContext<?> ctx) {
		...
		ctx.getStack().pushInt(instance.getMemory().getData().readByte(field.getOffset()));
