 ...
 * @author xDark
 */
public final class GetFieldDoubleProcessor implements InstructionProcessor<VMFieldInsnNode> {
	...
	public Result execute(VMFieldInsnNode insn, ExecutionContext<?> ctx) {
		...
		ctx.getStack().pushDouble(Double.longBitsToDouble(instance.getMemory().getData().readLong(field.getOffset())));
