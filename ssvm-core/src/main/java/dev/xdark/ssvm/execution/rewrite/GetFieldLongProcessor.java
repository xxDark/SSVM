 ...
 * @author xDark
 */
public final class GetFieldLongProcessor implements InstructionProcessor<VMFieldInsnNode> {
	...
	public Result execute(VMFieldInsnNode insn, ExecutionContext<?> ctx) {
		...
		ctx.getStack().pushLong(instance.getMemory().getData().readLong(field.getOffset()));
