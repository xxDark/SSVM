 ...
 * @author xDark
 */
public final class GetFieldCharProcessor implements InstructionProcessor<VMFieldInsnNode> {
	...
	public Result execute(VMFieldInsnNode insn, ExecutionContext<?> ctx) {
		...
		ctx.getStack().pushInt(instance.getMemory().getData().readChar(field.getOffset()));
